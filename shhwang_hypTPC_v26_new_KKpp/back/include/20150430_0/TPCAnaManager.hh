...
public:
  ...
  void SetTPCData(G4int tpctr, G4int tpcpid, G4int tpcparentid, G4double tpcpx, G4double tpcpy,G4double tpcpz,G4double tpcpp,  G4int tpcqq, G4double tpcpm, G4double tpcde, G4double tpclen, G4int tpclay,
...
double linearFitter(const int np,
  ...
  *b=(-beta *AA + alpha*BB )/(alpha*gamma-beta*beta);
  ...
  double circleFit(const double *mX,const double *mY,const double *mZ, const int npoints, double* mXCenter, double* mYCenter, double* mRadius, double* Pz)
{
