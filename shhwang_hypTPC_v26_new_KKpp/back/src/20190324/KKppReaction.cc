...
//reactio No #3001 K- d -> K0 K-K-pp, K-K-pp -> LL -> p pi p pi reaction
void KKppReaction::KKpp_LL1(G4Event* anEvent){
  ...
  pGen->anaManager->SetPrimaryParticle(0,LPf1.x(),LPf1.y(),LPf1.z(),PionPlus->GetPDGMass()/GeV);
  pGen->anaManager->SetPrimaryParticle(1,LPf2.x(),LPf2.y(),LPf2.z(),PionMinus->GetPDGMass()/GeV);
  pGen->anaManager->SetPrimaryParticle(2,LPf3.x(),LPf3.y(),LPf3.z(),Proton->GetPDGMass()/GeV);
  pGen->anaManager->SetPrimaryParticle(3,LPf4.x(),LPf4.y(),LPf4.z(),PionMinus->GetPDGMass()/GeV);
  pGen->anaManager->SetPrimaryParticle(4,LPf5.x(),LPf5.y(),LPf5.z(),Proton->GetPDGMass()/GeV);
  pGen->anaManager->SetPrimaryParticle(5,LPf6.x(),LPf6.y(),LPf6.z(),PionMinus->GetPDGMass()/GeV);
...
//reactio No #3002 K- d -> K0 K-K-pp, K-K-pp -> LL reaction
//K0S and Lambda is directory gunned
void KKppReaction::KKpp_LL2(G4Event* anEvent){
  ...
  pGen->anaManager->SetPrimaryInfo(mm_d, mm_p, thetaK, theta_scat, theta_CM);
  pGen->anaManager->SetPrimaryParticle(0,LPKz.x(),LPKz.y(),LPKz.z(),Kaon0S->GetPDGMass()/GeV);
  pGen->anaManager->SetPrimaryParticle(1,LPL1.x(),LPL1.y(),LPL1.z(),Lambda->GetPDGMass()/GeV);
  pGen->anaManager->SetPrimaryParticle(1,LPL2.x(),LPL2.y(),LPL2.z(),Lambda->GetPDGMass()/GeV);
...
int Nbeam_JAM=0;
//reactio No #3101 JAM input
void KKppReaction::JAMInput(G4Event* anEvent){
  
  G4double  mom_kp_x, mom_kp_y, mom_kp_z;
  ...
  for(int inp=0; inp<np_JAM; ++inp){
    ...
    else if(pid_JAM==311) ptmp=particleTable->FindParticle("kaon0");
    else if(pid_JAM==-311) ptmp=particleTable->FindParticle("anti_kaon0");
    else if(pid_JAM==321) ptmp=particleTable->FindParticle("kaon+");
    else{
    ...
    pGen->anaManager->SetPrimaryParticle(inp,LP.x(),LP.y(),LP.z(),ptmp->GetPDGMass()/GeV);
    //pGen->anaManager->SetPrimaryVertex(0,LPos.x(),LPos.y(),LPos.z());
