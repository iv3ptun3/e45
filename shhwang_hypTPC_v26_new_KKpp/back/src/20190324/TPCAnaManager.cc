    ...
    */
    ...
    for( G4int i=0; i<tpctrNum; i++){
      ...
      anaRoot.FillTPCData(tpcData[i].tpcpx,
			  ...
			  tpcData[i].tpcpp,
			  tpcData[i].tpcpid, tpcData[i].tpcparentid,
			  tpcData[i].tpcqq,
...
*/
 ...
 // G4cout<<  atoi( env_pad_num_in.c_str() )<<G4endl;
 // G4cout<<  atoi( env_pad_num_out.c_str() )<<G4endl;

    // G4int pad_in_num =  atoi( env_pad_num_in.c_str() );
    // G4int pad_out_num = atoi( env_pad_num_out.c_str() );

 // const G4int pad_in_num;
 // pad_in_num = const int(  atoi( env_pad_num_in.c_str() ));
 // const G4int pad_out_num;
 // pad_out_num = const int( atoi( env_pad_num_out.c_str() ));

 // const G4int pad_in_num=10;
 // const G4int pad_out_num=20;
 ...
 /*
    ...
    G4double pad_gap=1;
    G4double pad_in_width=2.5; //in 2mmx9
    G4double pad_out_width=2.5; //out 2mmx16
 */
	...
	//      G4cout<<"pad width:"<<new_pad_width<<G4endl;
	...
	//	G4cout<<"out iLay:"<<iLay<<G4endl;
	//	G4cout<<"out pad width:"<<seg_width[iLay]<<G4endl;
	//	delta_x=0;
      }
    }
    ...
    //    G4cout<<"out iLay:"<<iLay<<G4endl;
    //    G4cout<<"out pad width:"<<seg_width[iLay]<<G4endl;
    //    G4cout<<"delta x:"<<delta_x<<G4endl;


    //G4cout<<"angle:"<<ang_sh<<G4endl;
    //    G4cout<<"arch length:"<<delta_x<<":"<<arc_sh<<G4endl;
    //    G4cout<<"sh_rho:"<<sh_rho<<G4endl;
    //    G4cout<<"pad_in:"<<pad_in[iLay]<<G4endl;
    //    G4cout<<"pad_out:"<<pad_out[iLay]<<G4endl;
    //    G4cout<<"iLay:"<<iLay<<G4endl;

    //    if(delta_x>=0 && delta_x>pad_in_width/2){
    //      delta_x=pad_in_width/2-delta_x;
    //    }
    //    G4cout<<"re cal delta:"<<delta_x<<":"<<arc_sh<<G4endl;
    //if(delta_x<0 && delta_x > (-pad_in_width/2)){
    //      delta_x=pad_in_width/2;
    //    }

///include saho-san's code 
    //    ResHypTPC reshyptpc(double pad_size=2., double pad_length=10., double threshold=0.1, double diff_T=0.18, double smearing=0);
...
//pad and particle angle
    ...
    //    G4cout<<"dxdz"<<dxdz_track<<G4endl;
    //    G4cout<<acos((sh_pos.getX()*mom.getX()+sh_pos.getZ()*mom.getZ())/(sqrt(sh_pos.getX()*sh_pos.getX()+sh_pos.getZ()*sh_pos.getZ())*sqrt(mom.getX()*mom.getX()+mom.getZ()*mom.getZ())))*180/3.141592654<<G4endl;//pad and particle angle
    //    tpc.getXDeviation(n_electron, n_pad, x_rms, x_track, y_track, dxdz_track, dydz_track);
    ...
    //    ResHypTPC::~ResHypTPC();
    //    G4cout<<"----------------res program check----------------"<<G4endl;
    //    G4cout<<"resolution:"<<compx<<":"<<compy<<G4endl;
    //    G4cout<<"n_electron:"<<n_electron<<G4endl; 
    //    G4cout<<"n_pad:"<<n_pad<<G4endl;  
    //    G4cout<<"x_rms:"<<x_rms<<G4endl;    
    //    G4cout<<"x_track:"<<x_track<<G4endl;    
    ...
    //    double dxdz_track=0;
    //    double dydz_track=0;
    //    reshyptpc.getXDeviation(&n_electron, &n_pad, &x_rms,  x_track,  y_track, dxdz_track=0, dydz_track=0);
    //    G4cout<<n_electron<<":"<<pos.getY()<<G4endl;
    //getXDeviation(int &n_electron, int &n_pad, double &x_rms, double x_track, double y_track, double dxdz_track=0, double dydz_track=0)
    //    G4cout<<compy<<":"<<pos.getY()<<G4endl;
...
void TPCAnaManager::SetTPCData(G4int tpctr2, G4int tpcpid2, G4int tpcparentid2, G4double tpcpx2, G4double tpcpy2,G4double tpcpz2,G4double tpcpp2,  G4int tpcqq2, G4double tpcpm2, G4double tpcde2, G4double tpclen2, G4int tpclay2, 
