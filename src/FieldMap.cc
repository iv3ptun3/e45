...
FieldMap::Initialize( void )
{
  ...
  if( !( ifs >> m_nx >> m_ny >> m_nz >> m_x0 >> m_y0 >> m_z0 >> m_dx >> m_dy >> m_dz ) ){
  ...
  while( ifs.good() ){
    if( line++%1000000==0 )
      G4cout << "." << std::flush;
    ifs >> x >> y >> z >> bx >> by >> bz;
    G4int ix = G4int((x-m_x0+0.1*m_dx)/m_dx);
    G4int iy = G4int((y-m_y0+0.1*m_dy)/m_dy);
    G4int iz = G4int((z-m_z0+0.1*m_dz)/m_dz);
