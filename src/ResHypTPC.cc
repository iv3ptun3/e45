...
G4double
GetVerticalRes( G4double y, G4double dz, G4double de,G4double* par ){
  ...
  double dE_scale = sqrt(p3 + 1./(p4 + p5 * de));
/*
	double dl = y + 300;
	double val = sqrt( 
...
G4ThreeVector
GetResVector(G4ThreeVector pos,G4ThreeVector mom, G4double de,G4double* par_y, G4double* par_t){
	G4ThreeVector XZ(1,0,1);
	double PadAngle = atan2(-pos.x(),(pos.z()));
	double TrackAngle = PadAngle - atan2(-mom.x(),mom.z());
  double dz = mom.y()/mom.mag();
	double res_y = GetVerticalRes(pos.y(), dz,de,par_y);
...
G4ThreeVector
GetSmearingVector(G4ThreeVector pos,G4ThreeVector mom,G4double de, G4double* par_y, G4double* par_t){
	auto res_vect = GetResVector(pos,mom,de,par_y,par_t);
